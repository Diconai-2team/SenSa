...
// ─── ⑮ 차트 ───
var CHART_MAX = 20, buf = { labels: [], current: [] };
...
function pushPowerChart(power) {
  ...
  buf.labels.push(now); buf.current.push(power.current);
  if (buf.labels.length > CHART_MAX) { buf.labels.shift(); buf.current.shift(); }
