...
REBASELINE_TICKS  = 500  # N 틱마다 baseline 재산출 (개념 표류 대응)
...
def detect_drift(device_id: str, metric: str, value: float) -> dict:
    """
    CUSUM 드리프트 탐지.
    ...
    with _lock:
        ...
        if st["mu"] is None:
            ...
            detected  = S_high > h or S_low > h
            direction = ""
            if S_high > h:
                direction    = "up"
                st["S_high"] = 0.0  # 알람 후 리셋
            elif S_low > h:
                direction   = "down"
                st["S_low"] = 0.0
