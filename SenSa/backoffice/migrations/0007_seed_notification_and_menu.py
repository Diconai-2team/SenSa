...
def seed(apps, schema_editor):
    ...
    # 알림 정책
    for idx, (code, name, rc_code, al_code, channels, recipients, msg) in enumerate(
        DEFAULT_POLICIES, start=1
    ):
...
def unseed(apps, schema_editor):
    NotificationPolicy = apps.get_model("backoffice", "NotificationPolicy")
    MenuPermission = apps.get_model("backoffice", "MenuPermission")
    NotificationPolicy.objects.filter(
        code__in=[p[0] for p in DEFAULT_POLICIES]
    ).delete()
