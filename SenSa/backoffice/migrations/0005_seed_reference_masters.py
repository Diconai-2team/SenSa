...
CODE_GROUPS = [
    ...
    ('DEVICE_TYPE', '장비 유형', 10, [
        ('GAS_SENSOR',   '유해가스 센서'),
        ('SMART_POWER',  '스마트 전력 시스템'),
        ('LOCATION_NODE','위치 노드'),
        ('FACILITY',     '설비'),
    ]),
    ('COMM_METHOD', '통신 방식', 20, [
        ('TCP',     'TCP'),
        ('MQTT',    'MQTT'),
        ('MODBUS',  'Modbus'),
        ('HTTP',    'HTTP'),
...
THRESHOLD_CATEGORIES = [
    ...
    ('TH_GAS', '유해가스', 'realtime,alarm', 10, [
        ...
        ('o2',  '산소 (저산소)', '%',     'under', 18.0,    16.0),    # under: 18%↓ 주의, 16%↓ 위험
        ('o2_high', '산소 (과산소)', '%',  'over',  21.5,    23.5),    # over: 21.5%↑ 주의, 23.5%↑ 위험
...
def seed(apps, schema_editor):
    ...
    for cat_code, cat_name, applies, sort_order, types in RISK_CATEGORIES:
        cat, _ = RiskCategory.objects.get_or_create(
            code=cat_code,
            defaults={
                'name': cat_name, 'applies_to': applies,
        ...
        for idx, (t_code, t_name, on_map) in enumerate(types, start=1):
            RiskType.objects.get_or_create(
                category=cat, code=t_code,
                defaults={'name': t_name, 'show_on_map': on_map, 'sort_order': idx * 10},
            )
    ...
    for cat_code, cat_name, applies, sort_order, items in THRESHOLD_CATEGORIES:
        ...
        for ic, iname, unit, op, caution, danger in items:
            Threshold.objects.get_or_create(
                category=cat, item_code=ic,
                defaults={
                    'item_name': iname, 'unit': unit, 'operator': op,
                    'caution_value': caution, 'danger_value': danger,
                    'applies_to': applies,
                },
            )
...
def unseed(apps, schema_editor):
    ...
    apps.get_model('backoffice', 'RiskType').objects.filter(category__is_system=True).delete()
    ...
    apps.get_model('backoffice', 'Threshold').objects.filter(category__is_system=True).delete()
    apps.get_model('backoffice', 'ThresholdCategory').objects.filter(is_system=True).delete()
