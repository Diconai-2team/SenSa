...
def _resolve_qs(target: str, cutoff):
    ...
    if target == "sensor_data":
        ...
        return (
            SensorData.objects.filter(created_at__lt=cutoff)
            if hasattr(SensorData, "created_at")
            else None
        )
    if target == "worker_location":
        ...
        return (
            WorkerLocation.objects.filter(timestamp__lt=cutoff)
            if hasattr(WorkerLocation, "timestamp")
            else None
        )
    if target == "alarms":
...
class Command(BaseCommand):
    help = (
        "운영 데이터 보관 정책 (DataRetentionPolicy) 에 따라 누적 데이터를 삭제합니다."
    )

    def add_arguments(self, parser):
        parser.add_argument(
            "--dry-run", action="store_true", help="실제 삭제하지 않고 대상 건수만 출력"
        )
        parser.add_argument(
            "--target", type=str, default="", help="특정 target 만 처리 (예: alarms)"
        )
    ...
    def handle(self, *args, **options):
        ...
        for policy in qs:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"  [ok] {policy.target}: {deleted}건 삭제")
            )
