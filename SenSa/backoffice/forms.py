...
class UserCreateForm(forms.Form):
    ...
    def clean_username(self):
        ...
        if not LOGIN_ID_RE.fullmatch(v):
            raise forms.ValidationError("아이디는 영문 또는 숫자만 입력할 수 있습니다.")
...
class _MasterFormBase(forms.Form):
    ...
    def clean_code(self):
        ...
        if len(v) > self.code_max_len:
            raise forms.ValidationError(
                f"{self.code_label}는 {self.code_max_len}자 이하로 입력해 주세요."
            )
    ...
    def clean_name(self):
        ...
        if len(v) > self.name_max_len:
            raise forms.ValidationError(
                f"{self.name_label}은 {self.name_max_len}자 이하로 입력해 주세요."
            )
        return v
    ...
    def clean(self):
        ...
        if op and cv is not None and dv is not None:
            if op == "over" and dv <= cv:
                self.add_error(
                    "danger_value",
                    '판단 조건 "초과" 일 때 위험값은 주의값보다 커야 합니다.',
                )
            elif op == "under" and dv >= cv:
                self.add_error(
                    "danger_value",
                    '판단 조건 "이하" 일 때 위험값은 주의값보다 작아야 합니다.',
                )
        ...
        if cat and ic:
            ...
            if qs.exists():
                self.add_error(
                    "item_code", "이 분류 안에 이미 동일 측정 항목이 존재합니다."
                )
        return cleaned
...
class MenuPermissionUpdateForm(forms.Form):
    ...
    def clean_role(self):
        ...
        if v not in {"admin", "operator"}:
            raise forms.ValidationError(
                "역할은 admin 또는 operator 만 변경 가능합니다."
            )
        return v
    ...
    def _parse_dt(self, raw, field_name):
        ...
        for fmt in ("%Y-%m-%dT%H:%M", "%Y-%m-%d %H:%M", "%Y-%m-%d"):
            try:
                ...
                continue
        raise forms.ValidationError(
            "날짜 형식이 올바르지 않습니다 (예: 2026-05-01T09:00)."
        )

    def clean_published_from(self):
        return self._parse_dt(
            _strip_or_blank(self.cleaned_data.get("published_from")), "from"
        )

    def clean_published_to(self):
        return self._parse_dt(
            _strip_or_blank(self.cleaned_data.get("published_to")), "to"
        )
