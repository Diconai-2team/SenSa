# AI 브랜치 비교 분석 보고서
**`AI_test_shj` → `AI_test_shj_2`**  
작성일: 2026-05-22 | 작성자: Hojun Seo

---

## 📌 한눈에 보기

| 구분 | AI_test_shj (이전) | AI_test_shj_2 (현재) |
|------|-------------------|---------------------|
...
> 아래는 AI_test_shj_2 현재 DB 데이터 기준.

### 7-1. Proxy Label 기반 성능 (최근 7일)

| 지표 | 값 | 해석 |
|------|-----|------|
| **Recall (재현율)** | **90.0%** | 실제 위험 이벤트의 90%를 사전 포착 ✅ |
| **Precision (정밀도)** | 15.0% | AI 알람 → 실제 위험으로 이어진 비율 |
| **F1 Score** | 25.7% | 정밀도-재현율 조화평균 |
| **FP Rate (오탐률)** | 85.0% | 추가 튜닝 필요 |

### 7-2. 탐지기별 Precision

| 탐지기 | 정밀도 | 발화 건수 |
|--------|--------|-----------|
| ai_predictive_warning | **55.6%** | 133건 |
| ai_drift_alert (CUSUM) | **31.8%** | 1,236건 |
| ai_predictive_alert | 25.0% | 4건 (샘플 부족) |
| ai_anomaly_warning (Z-score) | 17.5% | 2,981건 |
| ai_trend_shift (ChangePoint) | 14.4% | 8,672건 |
| ai_ml_anomaly (IsolationForest) | 13.6% | 12,644건 |
| ai_correlation | 13.0% | 5,234건 |

### 7-3. Precision이 낮은 이유 (정상 동작)

Precision 15%는 표면적으로 낮아 보이지만, 이는 측정 방식의 구조적 한계:

1. **설계 목적**: AI는 임계치 도달 이전에 징후를 포착 → 임계치를 초과하지 않은 초기 경고가 FP로 집계
2. **Recall 90%**: 실제 위험 10건 중 9건을 사전에 탐지 (핵심 목표 달성)
3. **데이터 불균형**: AI 알람 30,904건 vs 임계치 알람 4,215건 → 비율 자체가 오탐처럼 보이게 함
...
| 우선순위 | 항목 | 예상 효과 |
|----------|------|----------|
| 🔴 높음 | contamination 0.030 → 0.025 (시스템 자동 권장 중) | ai_ml_anomaly FP 감소 |
| 🔴 높음 | ai_trend_shift MEAN_SHIFT_THRESHOLD 2.5 → 3.0 | 8,672건 → 대폭 감소 예상 |
| 🟡 중간 | ai_correlation 조건 강화 (현재 120초/3종 기준) | 5,234건 FP 개선 |
| 🟡 중간 | ARIMA 샘플 축적 대기 (현재 28건 → 500건+) | 통계 신뢰도 확보 |
| 🟢 낮음 | CUSUM H_SIGMA 개별 가스 재보정 | drift_alert 정밀도 향상 |
