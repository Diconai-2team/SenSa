...
GAS_OU_PARAMS = {
    ...
    "co2": {"theta": 0.08, "sigma": 25,     "min": 300, "max": 40000 },  # 카탈로그: 0~40,000 ppm
...
def _ou_step(prev: float, center: float, theta: float, sigma: float,
...
def generate_gas(tick: int, mode: str = "mixed",
    ...
    if mode == "danger":
        g = {
            ...
            "co2": 5000 + random.random() * 3000,   # caution 1000, danger 5000 → 5000~8000
            "o2":  12   + random.random() * 3.5,    # 저산소 위험 (caution 18, danger 16 미만) → 12~15.5
            "no2": 5.0  + random.random() * 3,      # caution 3, danger 5 → 5~8 (B-1 후속)
            "so2": 5.0  + random.random() * 3,      # caution 2, danger 5 → 5~8 (B-1 후속)
            "o3":  0.1  + random.random() * 0.1,    # caution 0.05, danger 0.1 → 0.1~0.2 (B-1 후속)
            "nh3": 50   + random.random() * 30,     # caution 25, danger 50 → 50~80 (B-1 후속)
...
POWER_EVENTS = {
    "current": {"prob": 1/50, "duration": 6, "active_center": 18, "active_sigma": 0.8},
}


def generate_power(tick: int, mode: str = "mixed",
                   prev_state: dict | None = None) -> dict:
    """
    ...
    for key, params in POWER_OU_PARAMS.items():
        ...
        is_event_active = (
            mode == "mixed"
            and key in POWER_EVENTS
            and events.get(key, 0) > 0
        )
        if is_event_active:
            ev = POWER_EVENTS[key]
            target_center = ev["active_center"]
