...
async def lifespan(app: FastAPI):
    ...
    startup:
      - app.state.scenario 초기화 (레거시 모드)
      - app.state.scenario_state 초기화 (5차 세션: R&D 토글 + phase ramp)
    ...
    app.state.scenario_state = ScenarioState()   # 5차 세션 — 5 device 토글 상태
    ...
    print(f"[main] 시나리오 토글 device: {sorted(SCENARIO_DEVICE_MAP.keys())}")
...
# ═══════════════════════════════════════════════════════════
# R&D 시나리오 토글 — 5차 세션 (v2.1 §11)
# ═══════════════════════════════════════════════════════════
#
# 5 device (sensor_01~03, power_01~02) 의 독립 토글.
# 토글 ON → phase 0→1→2→3→4 자동 ramp.
# 토글 OFF → phase 5→0 복귀.
# 매핑된 device 는 레거시 모드 (mixed/danger) 무시 — 토글 우위.

@app.post("/anomaly/toggle")
async def anomaly_toggle(device_id: str, state: bool):
    """
    5 device 중 하나의 토글 ON/OFF.

    호출 예:
      curl -X POST 'http://127.0.0.1:8001/anomaly/toggle?device_id=sensor_01&state=true'
      curl -X POST 'http://127.0.0.1:8001/anomaly/toggle?device_id=power_02&state=false'
    """
    result = app.state.scenario_state.toggle(device_id, state)
    if not result.get("ok", True):
        raise HTTPException(status_code=400, detail=result)
    print(f"[main] anomaly toggle: {device_id} → {state}")
    return result


@app.post("/anomaly/clear-all")
async def anomaly_clear_all():
    """5 device 전부 OFF + phase 0 즉시 리셋 (복귀 phase 5 거치지 않음)."""
    n = app.state.scenario_state.clear_all()
    print(f"[main] anomaly clear-all: {n}개 device 가 OFF 처리됨")
    return {"ok": True, "previously_on": n}


@app.get("/anomaly/state")
