...
async def post_sensor_data(
    ...
    Args:
        ...
        labels: (5차 세션) R&D 시나리오 라벨 (선택)
            {"scenario_id": str|None, "expected_phase": int|None, "expected_status": str|None}
            None 이면 payload 에 포함 안 됨 (Django 가 라벨 컬럼 null 로 저장).
    ...
    # 5차: 라벨 동봉 (None 값은 그대로 보내도 Django 가 null 저장)
    if labels is not None:
        payload["scenario_id"]     = labels.get("scenario_id")
